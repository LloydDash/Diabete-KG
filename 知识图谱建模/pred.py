...
# 数据预处理
df_y=pd.read_excel(os.path.join(cur_dir, 'train\\drug_antiDiabetes_code.xlsx'),header=None)
X = pd.read_excel(os.path.join(cur_dir, 'train\\diabetes_code2.xlsx',header=None)).values
y_all=df_y.values
...
X_pred = pd.read_excel(os.path.join(cur_dir, 'train\\disease_diabetes_code.xlsx'),header=None).values
